...
const Suggestions = ({data, setFilter}) => (
  <ul id="suggestions">
    {data.map((dat, key) => (
      <li key={key}
        onClick={() => setFilter(dat.key)}>{dat.key}</li>
    ))}
  </ul>
)
...
const SearchBox = ({data, placeholder}) => {
  ...
  const onChange = (value) => {
    if(value){
      ...
      const suggest = data.filter(i => String(i.key).toLowerCase().includes(String(value).toLowerCase()))
  ...
  return (
    <div id="searchBox">
      <h3 style={{padding: '0px', margin:'2px'}}>
        {placeholder}:
      </h3>
      <input
        value = {filter}
...
const SearchBar = ({authors, publishedYear, topics }) => {
  ...
  return (
    <div className="container">
      <h2> Filters
        {showFilters
          ? <AiOutlineUp onClick={() => setShowFilters(false)} className="dropDownArrow"/>
          : <AiOutlineDown onClick={() => setShowFilters(true)} className="dropDownArrow" />
        }
      </h2>
